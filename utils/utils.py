...
class SparsifyFn(nn.Module):
    ...
    def apply(self, x):
        temp = x.abs().gt(self.threshold)
        self.activated_sum += temp.sum()
        self.activation_sum += x.numel()
        return temp * x
...
class Distribution:
    def __init__(self, file_path, hidden_type):
        ...
        self.bin_centers, self.counts, self.var = histogram[f"{self.hidden_type}_centers"], histogram[self.hidden_type], histogram[f"{self.hidden_type}_var"]
...
class ActivationModule:
    ...
    def find_histogram(self, num_bins=10000, outlier_threshold=0.01):
        ...
        for key, acts in self.activations.items():
            ...
            self.histograms[f"{key}_var"] = torch.var(acts)
...
def get_sparse_model(model_name, device, histogram_path, **kwargs):
    from was.model import LlamaSparseForCausalLM, MistralSparseForCausalLM, LlamaSparseConfig, MistralSparseConfig
...
def get_layer_greedy_sparsities(layer_sparsities, results_dir):
    ...
    for layer, target_sparsity in enumerate(layer_sparsities):
        ...
        # print(f"layer{layer}, sparsity{target_sparsity}")
