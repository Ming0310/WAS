...
from was.mlp import _monkeypatch_mlp
from was.self_attn import _monkeypatch_self_attn
...
class SparseModelMixin:
    ...
    def load_greedy_sparsities(self, greedy_sparsity_path, greedy_sparsity_level, sparsities=None):
        if sparsities is None:
            layer_sparsity_levels = [greedy_sparsity_level] * len(self.model.layers)
        else:
            layer_sparsity_levels = sparsities
    ...
    def set_mlp_sparsity(self, sparsity, layer_idx=None):
        for i, layer in enumerate(self.model.layers):
    ...
    def set_self_attn_sparsity(self, sparsity, layer_idx=None):
        for i, layer in enumerate(self.model.layers):
    ...
    def set_uniform_sparsity(self, sparsity, layer_idx=None):
        self.set_mlp_sparsity(sparsity,layer_idx)
        self.set_self_attn_sparsity(sparsity,layer_idx)
